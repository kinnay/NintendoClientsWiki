...
The oldest node in a network has host migration ranking 0. Whenever a new node joins the network, the host migration ranking is updated. If a node has left the network, the host migration ranking of all nodes with a higher host migration ranking is decreased. The new node gets the highest host migration ranking in the network plus 1.
